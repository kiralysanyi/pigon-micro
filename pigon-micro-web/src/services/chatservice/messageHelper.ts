...
const sendGroupMessage = (message: string, chatID: number, cs: ChatService): Promise<number> => {
    return new Promise(async (resolve, reject) => {
        if (!cs.privKey) {
            reject("privKey not loaded");
            return;
        }
        ...
        cs.socket?.emit("message", { payload: JSON.stringify(encrypted), chatID, kGuid, type: "text" }, (messageId: number, error?: string) => {
            if (error) {
                throw error;
            }
...
const sendMessage = async (message: string, chatID: number, cs: ChatService): Promise<number> => {
    return new Promise<number>(async (resolve, reject) => {
        const startTime = new Date();
        const response = await api.get("/chat/" + chatID)

        if (cs.masterKey == undefined) {
            reject("Chatservice not inited properly");
            return;
        }
        ...
        console.log(chat)
        if (chat.type == "group") {
            resolve(await sendGroupMessage(message, chatID, cs))
            return;
        }
        ...
        cs.socket?.emit("message", { payload: JSON.stringify(encrypted), chatID, senderKeyId: sharedKey.senderKeyId, recipientKeyId: sharedKey.recipientKeyId, type: "text" }, (messageId: number, error?: string) => {
            // ack
            if (error) {
                throw error
            }
...
const sendFileMessage = async (chatID: number, cs: ChatService, sentCB?: (messageId: number) => void): Promise<{ type: "image" | "video", url: string }> => {
    if (!cs.masterKey) {
    ...
    if (chat.type == "group") {
        ...
        try {
            ...
            cs.socket?.emit("message", { payload: { assetId: data.assetId }, chatID, kGuid, type: data.type }, (messageId: number, error?: string) => {
                if (error) {
                    throw error
                }
                console.log("Sent file with message id: ", messageId)
    ...
    try {
        ...
        console.log("File uploaded: ", data)
        cs.socket?.emit("message", { payload: { assetId: data.assetId }, chatID, senderKeyId: sharedKey.senderKeyId, recipientKeyId: sharedKey.recipientKeyId, type: data.type }, (messageId: number, error?: string) => {
            if (error) {
                throw error
            }
            console.log("Sent file with message id: ", messageId)
