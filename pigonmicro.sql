...
--
-- Gép: mariadb:3306
-- Létrehozás ideje: 2026. Jún 11. 19:10
...
--
ALTER TABLE `chat_keys`
  ADD CONSTRAINT `usr_key` FOREIGN KEY (`userID`) REFERENCES `users` (`ID`) ON DELETE SET NULL ON UPDATE NO ACTION;
...
ALTER TABLE `group_keys`
  ...
  ADD CONSTRAINT `usr-gkey` FOREIGN KEY (`userId`) REFERENCES `users` (`ID`) ON DELETE SET NULL ON UPDATE NO ACTION;
...
ALTER TABLE `messages`
  ...
  ADD CONSTRAINT `msg-user` FOREIGN KEY (`senderID`) REFERENCES `users` (`ID`) ON DELETE CASCADE ON UPDATE NO ACTION;
